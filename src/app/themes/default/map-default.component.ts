...
export class MapDefaultComponent implements OnInit, OnChanges, OnDestroy {
  ...
  subDynamicLayerSubLayers: any;
	...
	mobileActive = true;
	get st() {
		console.log('Default C changes')
		return this._mapService.status
	}
  constructor(
  ...
  setActiveBasemap(view, basemap: string) {
	...
	ngOnChanges() {
		// if (this.st) {
		// 	this.InitiateDefaultView();
		// }
	}

  ngOnInit() {
		console.log('MAP default')
		this.mapStatusService.mapViewState$
			.subscribe((state) => {
				// add basic meta data
				...
				console.log("Map sate status", state);
				//this.InitiateDefaultView(state);
			});

  }

	async InitiateDefaultView(view) {
		...
		// return view
		//const view = this._mapService.getView();
		// view.when(v => {
		// 	console.log('2 Ready V', view.ready,  this._mapService.view,);
		// 	//this.view.then((view) => {
		...
		// });
		this.view = view;
		console.log('Ready V', view.ready,  this._mapService.view,);
		this._mapService.updateView(view)

		// set active basemaps based on url query params
		if (this.queryParams.basemap) {
			this.setActiveBasemap(view, this.queryParams.basemap);
		}
		...
		// 	this.search = await this.searchService.defaultSearchWidget(view);
