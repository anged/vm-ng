...
export class IdentifyService {
  ...
  async identify(layer: any) {
		const [IdentifyTask] = await loadModules([
			'esri/tasks/IdentifyTask'
		]);
    return new IdentifyTask(layer);
  }

  async identifyParams() {
		const [IdentifyParameters] = await loadModules([
  ...
  async identifyLayers(view) {
		const [on, all, IdentifyTask] = await loadModules([
			'dojo/on',
			'dojo/promise/all',
			'esri/tasks/IdentifyTask'
		]);
    const identifyParams = await this.identifyParams();
    ...
    const mapClickEvent = on(view, "click", (event) => {
      ...
      view.layerViews.items.forEach(item => {
        ...
        if ((item.layer.id !== "bufferPolygon") && (item.layer.id !== "allLayers_") && (!suspended) && (item.layer.listMode !== 'hide-children') && (item.layer.type !== 'group') && (item.layer.id !== 'FeatureSelection') && (item.layer.id !== 'AreaSelection') && (item.layer.popupEnabled)) {
          ...
          let defferedList = new IdentifyTask(item.layer.url);
					defferedList.execute(identifyParams).then((response) => {
