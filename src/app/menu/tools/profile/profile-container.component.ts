...
export class ProfileContainerComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    this.view = await this.mapService.getView();

    // add draw capabilities for temporary geometries
    this.draw = await this.profileToolService.initDraw(this.view);
  ...
  resetTools() {
    ...
    const action = this.draw.activeAction as esri.PolylineDrawAction;
