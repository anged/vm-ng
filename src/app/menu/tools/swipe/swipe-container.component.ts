...
export class SwipeContainerComponent implements AfterViewInit, OnDestroy {
  ...
  async initDomManipulation() {
    ...
    const projectsMIL = await this.ms.initDynamicLayerITV(url, 'projects-mil', 'new layers', 1);
    ...
    view.when(() => {
      const index = map.allLayers.length;
      ...
      projectsMIL.on('layerview-create', () => {
        ...
        timer(200, 400).pipe(
          ...
          .subscribe((lastIndex) => {
            ...
            map.reorder(this.projectsMIL, map.allLayers.length);
