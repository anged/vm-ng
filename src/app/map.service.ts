...
export class MapService {
  ...
  async viewMap(map: esri.Map, container ='map') {
  ...
  async initGroupLayer(id: string, name: string, listMode: string) {
  ...
  //for default themes
  async initDynamicLayer(layer: string, id: string = "itv", title: string = "itv", opacity = 1, sublayers = null, popupEnabled = true) {
  ...
  //for projects theme
  async initDynamicLayerITV(layer: string, id: string = "itv", name: string = "itv", opacity = 1) {
		const [MapImageLayer] = await loadModules([
			'esri/layers/MapImageLayer'
		]);
    return new MapImageLayer({
  ...
  async initSubAllDynamicLayers(layer: string, id: string = "itv", name: string = "itv", opacity = 1, sublayers: any[]) {
		const [MapImageLayer] = await loadModules([
			'esri/layers/MapImageLayer'
		]);
		return new MapImageLayer({
  ...
  async initGraphicLayer(id: number, scale: any = {}) {
  ...
  async initFeatureSelectionGraphicLayer(name: string, maxScale, minScale, listMode = 'show') {
		const [GraphicsLayer] = await loadModules([
			'esri/layers/GraphicsLayer'
		]);
    return new GraphicsLayer({
  ...
  async initGraphic(type: string, name: String, attr: any, geom, scale: any = "", graphicLayer, size = '12px', style = 'solid') {
  ...
  async initFeatureSelectionGraphic(type: string, geometry, layer, attributes, size = '12px', style = 'solid') {
		const [Graphic] = await loadModules([
			'esri/Graphic'
		]);
		return new Graphic({
  ...
  async initSymbol(type: string, size: any, style: string) {
		const [SimpleMarkerSymbol, SimpleLineSymbol, SimpleFillSymbol, Color] = await loadModules([
  ...
  async initTiledLayer(layer: string, name: string, visible = true) {
  ...
  async initFeatureLayer(layer: string, opacity = 1, index: number) {
  ...
  async initCommonFeatureLayer(layer: string, opacity = 1, id: number, title, symbolType) {
		const [FeatureLayer, Renderer] = await loadModules([
			'esri/layers/FeatureLayer',
			'esri/renderers/Renghderer'
		]);
		return new FeatureLayer({
      url: layer,
      id: 'feature-' + id,
  ...
  addToMap(response, queryParams) {
    ...
    if (!isInCache) {
      response.subscribe(json => {
				this.createSubLayers(json, queryParams);
      });
    } else {
	...
	async createSubLayers(json, queryParams) {
		...
		const layer =  await this.initSubAllDynamicLayers(MapOptions.mapOptions.staticServices.commonMaps, "allLayers", "Pagalbiniai sluoksniai", 0.8, subLayers);
		this.map.add(layer);
		//check other url params if exists
		//activate layer defined in url query params
		this.activateLayersVisibility(this.view, queryParams, this.map);
  ...
  pickMainThemeLayers(layer, key, queryParams, popupEnabled = true, groupLayer: any = false) {
    ...
    if (!isInCache) {
      ...
      response.subscribe(
        (json: any) => {
          if (!json.error) {
						this.initThemeLayers(layer, popupEnabled, key, groupLayer, json, queryParams)
          }
	...
	async initThemeLayers(layer, popupEnabled, key, groupLayer, json, queryParams) {
		// add dyn layers
		let sublayersArray = this.getSubDynamicLayerSubLayers(json.layers);
		let dynamicLayer = await this.initDynamicLayer(layer.dynimacLayerUrls, key, layer.name, layer.opacity, sublayersArray, popupEnabled)
  ...
  async runQueryByGeometry(urlStr: string, geometry: any) {
    const query = await this.addQuery();
    const queryTask = await this.addQueryTask(urlStr);
  ...
  pickCustomThemeLayers(response, layer, key, queryParams, groupLayer, serviceKey, symbolType = 'simple-fill') {
    ...
    response.subscribe(json => {
			this.addFeature(layer, key, serviceKey, groupLayer, symbolType, json, queryParams);
    });
  }

	async addFeature(layer, key, serviceKey, groupLayer, symbolType, json, queryParams) {
		//(layer: string, opacity, index: number, id: number, title)
		let feature = await this.initCommonFeatureLayer(layer.dynimacLayerUrls + '/' + serviceKey, layer.opacity, key, layer.name, symbolType);
		if (groupLayer) {
  ...
  async centerZoom(view: any, params: any) {
  ...
  async initLayerListWidget(view, container: HTMLElement) {
  ...
  async initSubLayerListWidget(view, map) {
		const [LayerList] = await loadModules([
			'esri/widgets/LayerList'
		]);
  ...
  async customBasemaps(layersArray: any[]) {
		console.log('basemaps', layersArray)
  ...
  async addQuery() {
    ...
    return new Query() as esri.Query ;
  }

  async addQueryTask(url: string) {
  ...
  async getExtentByPoints(xArray: number[], yArray: number[]) {
